...
const AddBlog = () => {
  ...
  useEffect(() => {
    ...
    return (
      <>
        <Toaster />
        <div className={`md:relative transition-all duration-300 top-0 right-0 absolute md:w-full w-[80%] md:p-0 p-4 md:translate-x-0 bg-zinc-900 ${nav ? 'translate-x-0' : 'translate-x-[100%]'} h-full`}>
          <div className="flex md:hidden items-center justify-center absolute w-20 rounded-l-full border-l h-14 left-[-70px] top-0 bg-zinc-900">
            <RiMenu3Line onClick={() => setNav((prev) => !prev)} color="white" size={25} className="cursor-pointer font-bold" />
          </div>
